...
> **ℹ️ About the example outputs in this walkthrough:** The PIDs, process
> names, hashes, and DLL lists shown are *illustrative* — they were captured
> from one specific memory sample. **Your actual values will differ** depending
> on which memory sample you download. That is expected and correct: this lab
> teaches the Volatility *method* (imageinfo → profile → pslist → pstree →
> psscan → dlllist), which works on any Windows memory dump. See
> `docs/evidence-sources.md` for free, official samples to download.
...
### 2. Obtain Your Memory Evidence

A Windows memory dump **cannot be generated by this lab** — it must be
captured from a real (or virtual) Windows system. Download a free,
pre-captured sample and place it at `evidence/memory.raw`:

1. Open **`docs/evidence-sources.md`** and pick a sample (the Digital Corpora
   M57 set or the Cridex XP sample are good starting points).
2. Download and decompress it.
3. Save it as `evidence/memory.raw` (any filename works — just adjust the
   commands below, which assume `memory.raw`).

```bash
# On your host machine — confirm your sample is present
ls -lh evidence/memory.raw
```

The file size depends on the sample you chose (typically 50 MB – 1 GB). The
analysis steps below work regardless of the specific sample.
